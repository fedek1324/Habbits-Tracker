...
interface IntegrationPannelProps {
  state: GoogleState;
  onGoogleStateChange: (state: GoogleState) => void;
...
const IntegrationPannel: React.FC<IntegrationPannelProps> = ({ state, onGoogleStateChange, onRefreshTokenChange, onAccessTokenChange }) => {
  ...
  const isError = state === GoogleState.ERROR;
  ...
  return (
    <div /*style={{ height: "152px" }}*/>
      <div className="max-w-md mx-auto space-y-4">
        ...
        {/* <!-- Error State --> */}
        {isError && (<div className="bg-gradient-to-r from-red-50 to-pink-50 border border-red-200 rounded-2xl p-5 shadow-sm">
          <div className="flex items-center gap-3 mb-3">
            <svg className="w-6 h-6" viewBox="0 0 24 24">
              <path
                fill="#9ca3af"
          ...
          <div className="flex gap-2">
            <button onClick={handleRepeatButtonClick} className="bg-red-600 hover:bg-red-700 text-white text-sm px-4 py-2 rounded-lg font-medium transition-colors duration-200">
              Repeat
            </button>
          </div>
        </div>)}
